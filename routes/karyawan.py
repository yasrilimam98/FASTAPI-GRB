from schemas.karyawan import KaryawanSchema, Karyawans, UserLogin, UserReg
...
@karyawan.get('/Karyawan/all', response_model=Karyawans, tags=["documentation"], 
             description="Menampilkan semua data")
async def find_all_karyawan(limit: int = 10, offset: int = 0):
    query = Karyawan.select().offset(offset).limit(limit)
    data = conn.execute(query).fetchall()
    response = {"limit": limit, "offset": offset, "data": data}
    return response

@karyawan.post('/register', status_code=201)
...
def login(auth_details: UserLogin):
    user = None
    for x in users:
        if x['email_karyawan'] == auth_details.email_karyawan:
            user = x
            break
    
    if (user is None) or (not auth_handler.verify_password(auth_details.password, user['password'])):
        raise HTTPException(status_code=401, detail='Invalid email and/or password')
    token = auth_handler.encode_token(user['email_karyawan'])
    return { 'token': token }
